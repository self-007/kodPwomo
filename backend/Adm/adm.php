...
function getOrdersDataByUniversityId($universityId) {
    ...
    global $connection;
    $stmt = $connection->prepare("SELECT products.name as product_name, products.picture as product_picture, users.name as user_name, orders.order_id, orders.date, orders.status, orders.qnt, orders.price, salle.salle_name as salle_name FROM products JOIN orders ON orders.id_product = products.id JOIN users ON users.id_unique = orders.id_user JOIN salle ON salle.id = adresse_id WHERE products.id_university = :university_id AND salle.id_university = :university_id");
    $stmt->bindParam(':university_id', $universityId);
    $stmt->execute();
    $nbrs = $stmt->rowCount();
    if($nbrs === 0){
